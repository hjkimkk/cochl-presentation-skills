...
description: Creates production-grade Cochl presentation collateral from minimal input — slide decks (investor, B2B sales, corporate strategy/roadmap, QBR/board, market & competitive, plus team decks for Data/Design/Dev/Research), documents, editable-SVG brochures, and press/social media banners. Follows the Cochl brand system (near-black + indigo + purple→blue glow, IBM Plex Sans/Mono). Use whenever the user wants to build a Cochl deck, presentation, pitch, brochure/제품 소개서, or social/press banner (LinkedIn/YouTube/Medium/Notion/OG).
license: Complete terms in LICENSE.txt
---

This skill builds Cochl-branded collateral — decks, documents, brochures, and social/press banners — from raw user content. It is organized as a small router over a **template registry** and a shared **brand core**, so it scales to many templates without bloat.
...
## Routing procedure
1. **Identify the artifact type** from the request: slide deck / presentation → `decks/`; developer or reference document → `docs/`; brochure / 제품 소개서 / 1-pager / leaflet → `brochure/`; social or press banner (LinkedIn/YouTube/Medium/Notion/OG, "press kit") → `social-kit/`.

...
A quote's attribution is one citation, not several independent fields — name, title, **and** publication/company are marked `[NEEDS INPUT]` together or not at all (see `../brand-core.md` §1e). Never leave a real, unbracketed outlet name (e.g. "TechCrunch") sitting next to a bracketed placeholder name/title in the same quote.

Each of the 4 social platforms on page 5 is a separate unconfirmed field — never reuse the confirmed web domain (`www.cochl.ai`) or any other single guessed value as a shared handle across all of them; a handle is confirmed per platform or not at all (see `../brand-core.md` §1f).
...
**Guard — a citation's fields share one confirmation level.** Page 4's quote attribution is `[NEEDS INPUT] — Name, Title` and `[NEEDS INPUT] — Publication / Company` — both bracketed, never one bracketed and the other left as bare descriptive text (or, worse, a real outlet name). If a real quote is dropped in with only some fields confirmed, bracket the rest to match rather than leaving them as plain-looking copy; a citation that's part real, part placeholder, with no visual distinction between the two, is a worse fabrication risk than one left entirely unconfirmed (see `../brand-core.md` §1e).

**Guard — social handles are 4 independent unconfirmed fields, not one shared guess.** Page 5's social row ships as generic icon glyphs plus one row-level caption ("Confirm official handles") and its `NI()` marker — there is no per-platform handle text today, which is the safe default. If actual handle text is ever added per icon, each of the 4 platforms needs its own independent confirmation; never fill all 4 from one derived value (e.g. the confirmed `www.cochl.ai` domain) and present that as verified — a domain being confirmed doesn't make a handle confirmed (see `../brand-core.md` §1f).

...
- [ ] **NEEDS INPUT bracket-format check** — grep every occurrence of the marker text across the whole document and confirm each one is written exactly `[NEEDS INPUT]`, brackets included; flag any bare `NEEDS INPUT` with no brackets, even where the `.needs-input` class and styling are otherwise correctly applied (see §1b). An entire case being internally consistent is not a pass — one case rendering every marker unbracketed and another rendering every marker bracketed is still a divergence from the single canonical format. (Guards the market-competitive Case 1 vs. Case 2 divergence: Case 1's 32 markers all shipped as bare `NEEDS INPUT`, Case 2's 34 markers all shipped as `[NEEDS INPUT]`.)
...
- [ ] **Shared-layout reuse check** — for any repeated slide archetype used across multiple builds of the same deck type (e.g. `key-initiatives`), confirm the build reused the layout's registered class structure rather than standing up a new one; scaling the item count via a property like `grid-template-columns` is a valid adaptation of the same registered layout, a differently-structured replacement is not (see registry.md → Repeated slide-type layouts are canonical). (Confirms the QBR Q4 case: Q4 correctly reused Q2/Q3's `init-grid`/`init-card` classes for its 4-column, 14-card Key initiatives expansion — including full `[NEEDS INPUT]` coverage on every Owner field — rather than authoring a separate layout; this check formalizes that pattern as the required default across every deck type going forward.)
- [ ] **Cross-build figure-reuse check** — when generating a financial/quarterly-target chart for a new deck build (a new fiscal year, a new quarter, a fresh iteration of the same deck type), compare its numeric series against the equivalent chart from this session's other builds of the same deck type; flag any series where most values match a prior build's values verbatim or within a trivial rounding difference — recycling old illustrative/placeholder figures into a new period's chart is a fabrication risk even when captioned "illustrative," not acceptable reuse (see SKILL.md Facts & fabrication → Cross-build / cross-fiscal-year figure reuse). (Guards the QBR case: Q2's 2026 build-plan quarterly financial targets and Q4's FY2027 financial-plan quarterly targets matched in 3 of 4 quarters exactly, with the fourth differing by only $0.1M.)

...
**"추천해줘" 시 자동 확정** — 사용자가 버전을 지정하지 않고 "추천"을 요청하면, 위 기준 표를 사용자에게 노출해 고르게 하지 **않는다**. 콘텐츠의 정보량·수치 비중·톤을 분석해 가장 적합한 버전 1개를 스킬이 내부적으로 바로 확정하고, 곧장 내용 채우기(작업 순서 3)로 진행한다. 확정된 버전은 결과 전달 시 **한 줄로만** 안내한다 — 예: "정보량이 많고 스펙 행이 있어 **V3 Editorial**로 진행했습니다."

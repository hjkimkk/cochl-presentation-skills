...
- **Closing pattern (dark) — Q&A / Thank-you:** `cochl.` logo **top-left** (per the original Figma — this slide, uniquely, places the logo top-left rather than the default bottom-left footer). Content stack: `Q & A` mono kicker (indigo) → bold **Thank you** → a **single combined presenter line** `{presenter} · {email} · {lab / institution}`, joined by ` · `. **Omit any item whose value is empty, its separator included** — never split the presenter info into a two-row label or a three-line list, and never render an empty field. Keep only the one standard purple→blue glow; **remove any decorative element not in the original Figma.**
- **Logo placement — dark surfaces only (overrides brand-core §3 / footer-chrome QA for this light deck).** The `cochl.` logo is the **white** wordmark (`assets/cochl-logo-white.png`), legible only on dark backgrounds. Place it **only on the dark slides — the Cover, the 8 section dividers, and the Q&A / Thank-you closer** (the closer positions its logo **top-left** per the Closing pattern above; the Cover and dividers keep it bottom-left). **Never put the logo in a light content slide's footer:** on the white body the white wordmark disappears and only the gradient dot shows. Light content slides keep just the text footer (`[Presentation Title]` mono, bottom-right) — **no footer logo.** brand-core's footer-chrome rule ("logo bottom-left on every non-cover slide") and its QA count therefore apply, for this deck, to the **dark slides only**; do not add a light-slide footer logo to satisfy that global count.

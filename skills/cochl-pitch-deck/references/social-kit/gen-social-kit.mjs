// Cochl press / social media kit — editable SVG at each platform's exact spec.
// Dark brand (near-black + indigo/purple glow), real cochl. logotype (white wordmark + gradient symbol), IBM Plex.
import { writeFileSync, mkdirSync } from 'node:fs';
const OUT='/Users/hyo/Desktop/cochl-social-kit'; mkdirSync(OUT,{recursive:true});
...
const GLOW='<radialGradient id="glow" cx="50%" cy="50%" r="50%"><stop offset="0" stop-color="#4b68ff" stop-opacity="0.55"/><stop offset="0.5" stop-color="#832bfb" stop-opacity="0.22"/><stop offset="1" stop-color="#4b68ff" stop-opacity="0"/></radialGradient>';
const SANS="'IBM Plex Sans','Helvetica Neue',Arial,sans-serif", MONO="'IBM Plex Mono','SF Mono',monospace";
function logo(x,y,s){return `<g id="logo" transform="translate(${x},${y}) scale(${s})"><path d="${SYMB}" fill="url(#cochlGrad)"/><path d="${WORD}" fill="#FFFFFF"/><circle cx="499.067" cy="102.831" r="10.9334" fill="#FFFFFF"/></g>`;}
function glow(cx,cy,rx,ry){return `<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="url(#glow)"/>`;}
function open(w,h){return `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg"><defs>${GRAD}${GLOW}</defs><g id="bg"><rect width="${w}" height="${h}" fill="#0b0b12"/></g>`;}
// tx(x,y,size,weight,fill,text,{mono,ls,anchor})
const tx=(x,y,size,weight,fill,text,{mono,ls,anchor}={})=>`<text x="${x}" y="${y}" font-family="${mono?MONO:SANS}" font-size="${size}" font-weight="${weight}" fill="${fill}"${ls?` letter-spacing="${ls}"`:''}${anchor?` text-anchor="${anchor}"`:''}>${text}</text>`;
const files={};

// 1 LinkedIn banner 1584×396 (profile/company cover)
{const W=1584,H=396;let s=open(W,H);
 s+=`<g id="glow-layer">${glow(1360,150,760,620)}</g>`;
 s+=`<g id="brand">${logo(120,118,0.62)}`;
 s+=tx(122,300,27,'400','#c9c9d4','Sound AI for the physical world');
 s+=`</g>`;
 s+=`<g id="tag">${tx(W-120,152,15,'500','#9aa1ac','PRESS · MEDIA KIT',{mono:1,ls:'2',anchor:'end'})}${tx(W-120,184,15,'400','#4b68ff','cochl.ai',{mono:1,anchor:'end'})}</g>`;
 s+='</svg>';files['cochl-linkedin-banner-1584x396.svg']=s;}

// 2 YouTube channel art 2560×1440 (safe area 1546×423 centered)
{const W=2560,H=1440,sx=(W-1546)/2,sy=(H-423)/2,cx=W/2;let s=open(W,H);
 s+=`<g id="glow-layer">${glow(cx,H/2,1200,900)}</g>`;
 s+=`<g id="brand">${logo(cx-330,sy+66,1.3)}`;
 s+=tx(cx,sy+300,40,'600','#ffffff','Creating ears for AI',{anchor:'middle'});
 s+=tx(cx,sy+352,22,'400','#9aa1ac','SOUND AI FOR THE PHYSICAL WORLD · cochl.ai',{mono:1,anchor:'middle',ls:'2'});
 s+=`</g>`;
 s+=`<g id="guides" opacity="0.6"><rect x="${sx}" y="${sy}" width="1546" height="423" fill="none" stroke="#4b68ff" stroke-width="2" stroke-dasharray="10 8"/>${tx(cx,sy-18,16,'500','#4b68ff','SAFE AREA — visible on all devices (delete this layer)',{mono:1,anchor:'middle',ls:'2'})}</g>`;
 s+='</svg>';files['cochl-youtube-channel-2560x1440.svg']=s;}

// 3 Medium banner 1500×750
{const W=1500,H=750;let s=open(W,H);
 s+=`<g id="glow-layer">${glow(1250,180,720,680)}</g>`;
 s+=`<g id="brand">${logo(110,108,0.7)}`;
 s+=tx(112,360,72,'700','#ffffff','Creating ears');
 s+=tx(112,448,72,'700','#4b68ff','for AI.');
 s+=tx(114,516,24,'400','#c9c9d4','Real-time sound intelligence for the physical world.');
 s+=`</g>`;
 s+=`<g id="foot"><line x1="112" y1="650" x2="1388" y2="650" stroke="#ffffff" stroke-opacity="0.12"/>${tx(112,690,17,'400','#9aa1ac','PRESS · MEDIA KIT',{mono:1,ls:'1'})}${tx(1388,690,17,'400','#4b68ff','www.cochl.ai',{anchor:'end'})}</g>`;
 s+='</svg>';files['cochl-medium-banner-1500x750.svg']=s;}

// 4 Notion cover 1500×600 (keep content in central band; Notion crops top/bottom)
{const W=1500,H=600;let s=open(W,H);
 s+=`<g id="glow-layer">${glow(1280,300,640,520)}</g>`;
 s+=`<g id="brand">${logo(110,232,0.62)}`;
 s+=tx(462,300,34,'600','#ffffff','Sound AI for the physical world');
 s+=tx(464,340,18,'400','#9aa1ac','PRESS · MEDIA KIT · cochl.ai',{mono:1,ls:'1.5'});
 s+=`</g>`;
 s+='</svg>';files['cochl-notion-cover-1500x600.svg']=s;}

// 5 OG / social share card 1200×630
{const W=1200,H=630,cx=W/2;let s=open(W,H);
 s+=`<g id="glow-layer">${glow(cx,H+120,900,640)}</g>`;
 s+=`<g id="brand">${logo(cx-300,150,1.17)}`;
 s+=tx(cx,400,44,'700','#ffffff','Creating ears for AI',{anchor:'middle'});
 s+=tx(cx,448,22,'400','#c9c9d4','Sound AI for the physical world',{anchor:'middle'});
 s+=tx(cx,540,16,'500','#4b68ff','cochl.ai',{mono:1,anchor:'middle',ls:'2'});
 s+=`</g>`;
 s+='</svg>';files['cochl-og-social-card-1200x630.svg']=s;}
